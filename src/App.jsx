...
const startWind = () => {
  ...
  filter.type = 'lowpass';
  filter.frequency.value = 300; // くぐもった風の音
  
  // 揺らぎ（風の強弱）をつけてよりリアルに
  windLfo = bgmAudioCtx.createOscillator();
  windLfo.type = 'sine';
  windLfo.frequency.value = 0.15; // ゆったり
  ...
  windGainNode.gain.value = 0.05; // 風の音量 (大きすぎないように調整)
...
const scheduleBgmNote = (beatNumber, time) => {
  ...
  let melodyArray = bgmMelody;
  let bassArray = bgmBass;
  let isMuffled = false; // エントランス用（こもった音フラグ）

  if (currentBgmType === 'creepy') {
    melodyArray = creepyMelody;
    bassArray = creepyBass;
  } else if (currentBgmType === 'shop' || currentBgmType === 'entrance') {
    melodyArray = shopMelody;
    bassArray = shopBass;
    if (currentBgmType === 'entrance') isMuffled = true;
  }
  ...
  if (mFreq > 0) {
    ...
    // 店内から漏れ聞こえるようなこもった音にする（フィルター調整）
    if (isMuffled) {
      const filter = bgmAudioCtx.createBiquadFilter();
      filter.type = 'lowpass';
      filter.frequency.value = 600; // 少し聞き取りやすくする
      gain.connect(filter);
      filter.connect(targetDest);
    } else {
    ...
    if (currentBgmType === 'creepy') {
      ...
      osc.frequency.linearRampToValueAtTime(mFreq - 5, time + 0.3);
      osc.connect(gain);
      ...
      osc.connect(gain);
      gain.gain.setValueAtTime(0, time);
      const peakVol = isMuffled ? 0.008 : 0.015; // こもっている時も少し音量を上げる
      gain.gain.linearRampToValueAtTime(peakVol, time + 0.05);
  ...
  if (bFreq > 0) {
    ...
    if (isMuffled) {
      const filter = bgmAudioCtx.createBiquadFilter();
      filter.type = 'lowpass';
      filter.frequency.value = 300; // 少し聞き取りやすくする
      gain.connect(filter);
      filter.connect(targetDest);
    } else {
      gain.connect(targetDest);
    ...
    if (currentBgmType === 'creepy') {
      ...
      gain.gain.setValueAtTime(0, time);
      const peakVol = isMuffled ? 0.01 : 0.02; // こもっている時も少し音量を上げる
      gain.gain.linearRampToValueAtTime(peakVol, time + 0.05);
      gain.gain.exponentialRampToValueAtTime(0.0001, time + 0.4);
      osc.start(time);
      osc.stop(time + 0.4);
...
const playBGM = (type = 'normal') => {
  ...
  currentBgmType = type;
  current16thNote = 0;
  nextNotetime = bgmAudioCtx.currentTime + 0.05;
  bgmScheduler();
};
...
const stopBGM = () => {
  isBgmPlaying = false;
  if (bgmTimerId) {
    cancelAnimationFrame(bgmTimerId);
  }
  stopWind(); // 風の音も一緒に止める
};
...
const playSound = (type) => {
  try {
    ...
    // 効果音もプツプツ鳴らないようにエンベロープ（音量変化）を調整
    if (type === 'doorbell') {
      const playBellTone = (baseFreq, startTime, duration, maxVol) => {
        // 金属感を出すための非整数倍の周波数比率（ベル特有の複雑な響き）
        ...
        ratios.forEach((ratio, index) => {
          ...
          bellOsc.stop(startTime + duration);
        });
      };

      // チャ・リ・リーンと和音が重なるように鳴らす
...
export default function App() {
  ...
  }, [coins, playCount, inventory, plateInventory, activePlateId, facilityInventory, emotionInventory, activeEmotion, staffInventory, activeStaffId, activeItems, soldInventory, soldPlateInventory, soldEmotionInventory, soldStaffInventory, shopLevel, shopTrust, hasFreeCoupon]);
  ...
  useEffect(() => {
    ...
    const checkMobile = () => {
      ...
      if (/android|ipad|playbook|silk/i.test(userAgent.toLowerCase()) || /iphone|ipod/i.test(userAgent.toLowerCase()) || 'ontouchstart' in window || navigator.maxTouchPoints > 0) {
        setIsMobile(true);
      }
    };
    checkMobile();
  ...
  useEffect(() => {
    if (isBgmEnabled && gameState !== 'closed') {
      if (gameState === 'entrance') {
        playBGM('entrance'); // エントランス用（こもったBGM＋風）
      } else if (gameState === 'playing' && currentEvent === 'trade') {
        stopBGM(); // 取引イベントのタイピング中はBGMを停止
      } else if (gameState === 'trade') {
        playBGM('creepy');
      } else if (gameState === 'shop' || gameState === 'sell' || gameState === 'inventory') {
        playBGM('shop'); // ショップ画面などでのんびりBGM
      } else {
        playBGM('normal');
      }
    } else {
  ...
  useEffect(() => {
    const unlockAudio = () => {
      ...
      // ユーザーがクリックしたタイミングでAudioContextのロックを確実に解除する
  ...
  const spawnWord = useCallback((isStopped = false) => {
    ...
    if (currentEvent === 'deer_staff' && !deerSenbeiSpawnedRef.current) {
      if (Math.random() < 0.25) {
        nextWord = DEER_SENBEI;
        deerSenbeiSpawnedRef.current = true;
      }
    }
    ...
    if (currentEvent === 'fire_event') {
      ...
      if ((isForceFire && Math.random() < 0.6) || (!isForceFire && Math.random() < 0.2)) {
        nextWord = { ...nextWord, isFire: true };
        fireBreadSpawnCountRef.current++;
      }
    }
  ...
  const startGame = (selectedCourse) => {
    ...
    deerSenbeiSpawnedRef.current = false;
    fireBreadSpawnCountRef.current = 0;
    ...
    const handleKeyDown = (e) => {
      ...
      if (isOnFire && currentWater) {
        ...
        if (e.key.toLowerCase() === expectedChar.toLowerCase()) {
          playSound('type');
          const newTypedCount = currentWater.typedCount + 1;
          ...
          if (newTypedCount === currentWater.word.romaji.length) {
            ...
            setCurrentWater(prev => ({ ...prev, typedCount: newTypedCount }));
          }
        } else {
          playSound('miss');
          setCombo(0);
      ...
      if (e.key.toLowerCase() === expectedChar.toLowerCase()) {
        ...
        if (newTypedCount === currentBread.word.romaji.length) {
          ...
          if (currentBread.word.isFire) {
            setIsOnFire(true);
            const waterWord = WATER_WORDS[Math.floor(Math.random() * WATER_WORDS.length)];
  ...
  const getMostRareItem = () => {
    ...
    check(inventory, SHOP_ITEMS, 'item');
    check(plateInventory, PLATE_ITEMS, 'plate');
    check(facilityInventory, FACILITY_ITEMS, 'facility');
    check(emotionInventory, EMOTION_ITEMS, 'emotion');
    check(staffInventory, STAFF_ITEMS, 'staff');

    return { item: rarest, category };
  };

  const handleTradeRefuse = () => {
    setGameState('result');
    if (!course.noCoin) setCoins(prev => prev + score);
  };
  ...
  const handleTradeAccept = () => {
    ...
    if (category === 'item') {
      ...
      setPlateInventory(prev => prev.filter(id => id !== item.id));
      if (activePlateId === item.id) setActivePlateId('plate_default');
      ...
      setEmotionInventory(prev => prev.filter(id => id !== item.id));
      if (activeEmotion === item.id) setActiveEmotion('normal');
    } else if (category === 'staff') {
      setStaffInventory(prev => prev.filter(id => id !== item.id));
      if (activeStaffId === item.id) setActiveStaffId('staff_panda');
    }

    setLostTradeItem(item);

    let potentialRewards = [
      { text: 'コイン 5,000 C', action: () => setCoins(prev => prev + 5000) },
      { text: 'コイン 10,000 C', action: () => setCoins(prev => prev + 10000) },
      { text: 'コイン 30,000 C', action: () => setCoins(prev => prev + 30000) },
      { text: 'コイン 50,000 C', action: () => setCoins(prev => prev + 50000) },
      { text: 'コイン 10 C', action: () => setCoins(prev => prev + 10) }, 
    ];

    const addUnowned = (allItems, ownedList, addAction) => {
      allItems.forEach(item => {
        if (item.price > 0 && !ownedList.includes(item.id)) {
          potentialRewards.push({
            text: `[レア] ${item.name}`,
            action: () => addAction(prev => [...prev, item.id])
          });
        }
      });
    };

    addUnowned(SHOP_ITEMS, inventory, setInventory);
    addUnowned(PLATE_ITEMS, plateInventory, setPlateInventory);
    addUnowned(EMOTION_ITEMS, emotionInventory, setEmotionInventory);
    addUnowned(STAFF_ITEMS, staffInventory, setStaffInventory);

    const shuffled = potentialRewards.sort(() => 0.5 - Math.random()).slice(0, 3);
    setTradeRewards(shuffled);
    setTradePhase('choices');
  };

  const handleTradeRewardSelect = (reward) => {
    reward.action();
    setGainedTradeReward(reward.text);
    setTradePhase('done');
  };

  const handleTradeFinish = () => {
    setGameState('result');
    if (!course.noCoin) setCoins(prev => prev + score);
  };
  ...
  const getStaffMessage = () => {
    ...
    const s = displayStaff.suffix;
    if (gameState === 'entrance') return `お店はここだ${s}！`;
    if (gameState === 'title') {
      if (currentEvent === 'deer_staff') return `期間限定でバイト中だ${s}！`;
      return `パン屋へようこそ${s}！`;
    }
  ...
  const InventoryItemRow = ({ item, isEquipped, onEquip, isToggleable = false, count = null }) => {
    return (
      <div className={`p-4 rounded-xl border-2 flex flex-col md:flex-row items-center justify-between gap-4 transition-colors ${isEquipped ? 'bg-amber-100 border-amber-400' : 'bg-slate-50 border-slate-200 hover:border-slate-300'}`}>
        <div className="flex items-center gap-4 text-center md:text-left w-full md:w-auto">
          {item.css ? (
            <div className={`w-16 h-8 rounded-[100%] mx-auto md:mx-0 shadow-md ${item.css}`}></div>
          ) : (
            <div className={`text-5xl`}>{item.emoji}</div>
          )}
          <div>
            <h3 className={`text-xl font-bold text-amber-900`}>
              {item.name} {isEquipped && <span className="text-xs bg-amber-500 text-white px-2 py-1 rounded-full ml-2 align-middle">使用中</span>}
            </h3>
            {item.desc && <p className={`text-sm text-amber-700`}>{item.desc}</p>}
          </div>
        </div>
        <div className="text-center md:text-right shrink-0">
          {count !== null ? (
            <span className="font-bold text-amber-800 bg-amber-200 px-4 py-2 rounded-full border-2 border-amber-300 shadow-sm">所持: {count} 個</span>
          ) : (
            <button disabled={!isToggleable && isEquipped} onClick={() => onEquip(item.id)} className={`px-6 py-2 rounded-full font-bold shadow-md transition-all ${isEquipped ? (isToggleable ? 'bg-rose-500 hover:bg-rose-400 text-white active:translate-y-1 border-b-4 border-rose-600 active:border-b-0' : 'bg-slate-400 text-white cursor-not-allowed') : 'bg-blue-500 hover:bg-blue-400 text-white active:translate-y-1 border-b-4 border-blue-600 active:border-b-0'}`}>
              {isEquipped ? (isToggleable ? '外す' : '使用中') : '装備する'}
            </button>
          )}
        </div>
      </div>
    ...
    <div className={`min-h-screen flex flex-col items-center justify-center p-4 font-sans selection:bg-orange-200 transition-colors duration-700 ${gameState === 'trade' ? 'bg-slate-950 text-slate-200' : 'bg-orange-50 text-amber-900'}`}>
      ...
      <style>{`
        ...
        @keyframes eventBanner {
          ...
          100% { transform: translate(-50%, -50%) scale(1.5); opacity: 0; visibility: hidden; }
        }
        .animate-event-banner {
          animation: eventBanner 3s forwards;
        }
      `}</style>
      ...
      <div className={`w-full max-w-4xl p-6 md:p-8 rounded-3xl relative overflow-hidden transition-colors duration-700 ${gameState === 'trade' ? 'bg-slate-900 border-4 border-purple-900 shadow-[0_0_50px_rgba(88,28,135,0.4)]' : 'bg-white border-4 border-orange-100 shadow-[0_8px_30px_rgb(0,0,0,0.08)]'}`}>
        ...
        <div className="absolute top-6 right-6 flex flex-col md:flex-row items-end md:items-center gap-3 z-20">
          {gameState !== 'closed' && (
            <button 
              ...
              className={`px-3 py-1.5 rounded-full border-2 font-black shadow-sm transition-colors text-xs md:text-sm ${gameState === 'trade' ? 'bg-slate-800 border-purple-800 text-purple-400' : isBgmEnabled ? 'bg-orange-100 border-orange-300 text-orange-600' : 'bg-slate-100 border-slate-300 text-slate-400'}`}
            >
          ...
          <div className="text-center pt-32 pb-20 flex flex-col items-center justify-center min-h-[400px]">
            ...
            {isMobile && (
              <div className="bg-blue-50 border-4 border-blue-300 p-4 rounded-xl mb-8 max-w-md shadow-md animate-pulse">
                ...
                <p className="text-blue-600 font-bold text-sm md:text-base leading-relaxed mb-4">
                  ごめん！このゲームはパソコン専用なんだ。<br />
                  これを理解した上でプレイしてね。
                </p>
                <label className="flex items-center justify-center gap-3 cursor-pointer bg-blue-100 p-2 rounded-lg border-2 border-blue-200 hover:bg-blue-200 transition-colors">
                  ...
                  <input 
                    type="checkbox" 
                    checked={isMobileConfirmed}
                    onChange={(e) => setIsMobileConfirmed(e.target.checked)}
            ...
            <button
              disabled={isMobile && !isMobileConfirmed}
              onClick={() => { setGameState('title'); playSound('doorbell'); }}
              className={`px-10 py-5 font-black text-2xl md:text-3xl rounded-full shadow-xl transition-all flex items-center gap-3 
                ${(!isMobile || isMobileConfirmed) 
                  ? 'bg-orange-500 hover:bg-orange-400 text-white border-b-8 border-orange-600 active:border-b-0 active:translate-y-2 animate-bounce hover:animate-none' 
                  : 'bg-slate-400 text-slate-100 border-b-8 border-slate-500 cursor-not-allowed'
                }`}
            >
        ...
        {gameState === 'title' && (
          <div className="text-center pt-28 pb-12">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <button onClick={() => setGameState('inventory')} className="px-8 py-3 bg-amber-500 hover:bg-amber-400 text-white font-black text-lg rounded-full shadow-lg border-b-4 border-amber-600 active:border-b-0 active:translate-y-1 transition-all flex items-center gap-2">
                <span className="text-2xl">🎒</span> インベントリ
              </button>
          ...
          <div className="pt-28 pb-12 px-2 md:px-10">
            ...
            {inventoryTab === 'items' && (
              <div className="grid gap-4 max-w-2xl mx-auto mb-10">
                {SHOP_ITEMS.filter(item => inventory.includes(item.id)).length === 0 ? <p className="text-center text-amber-700 font-bold">持っている便利アイテムはないよ！</p> : 
                SHOP_ITEMS.filter(item => inventory.includes(item.id)).map(item => (
                  <InventoryItemRow 
                    key={item.id} item={item} 
                    isEquipped={activeItems.includes(item.id)} onEquip={handleToggleItem} isToggleable={true}
                  />
                ))}
              </div>
            )}
            ...
            {inventoryTab === 'plates' && (
              <div className="grid gap-4 max-w-2xl mx-auto mb-10">
                {PLATE_ITEMS.filter(plate => plateInventory.includes(plate.id)).map(plate => (
                  <InventoryItemRow 
                    key={plate.id} item={plate} 
                    isEquipped={activePlateId === plate.id} onEquip={handleEquipPlate} 
                  />
                ))}
              </div>
            )}

            {inventoryTab === 'facilities' && (
              <div className="grid gap-4 max-w-2xl mx-auto mb-10">
                <p className="text-center text-amber-600 font-bold mb-4 text-sm bg-amber-100 py-2 rounded-xl">
                  ※ 設備はゲームプレイ中に 1, 2 キーを押して使う消費アイテムです。
                </p>
                {FACILITY_ITEMS.filter(item => facilityInventory.includes(item.id)).length === 0 ? <p className="text-center text-amber-700 font-bold">持っている設備はないよ！</p> : 
                FACILITY_ITEMS.filter(item => facilityInventory.includes(item.id)).map(item => {
                  ...
                  return <InventoryItemRow key={item.id} item={item} count={count} />;
                }).filter((v, i, a) => a.findIndex(t => (t.key === v.key)) === i)}
              </div>
            )}

            {inventoryTab === 'emotions' && (
              <div className="grid gap-4 max-w-2xl mx-auto mb-10">
                <InventoryItemRow 
                  item={{ id: 'normal', name: '感情なし', desc: '標準の状態に戻す', emoji: '🙂' }} 
                  isEquipped={activeEmotion === 'normal'} onEquip={() => { setActiveEmotion('normal'); playSound('type'); }} isToggleable={false}
                />
                {EMOTION_ITEMS.filter(item => emotionInventory.includes(item.id)).map(item => (
                  <InventoryItemRow 
                    key={item.id} item={item} 
                    isEquipped={activeEmotion === item.id} onEquip={() => { setActiveEmotion(item.id); playSound('type'); }} isToggleable={false}
                  />
                ))}
              </div>
            )}
            ...
            {inventoryTab === 'staffs' && (
              <div className="grid gap-4 max-w-2xl mx-auto mb-10">
                {STAFF_ITEMS.filter(staff => staffInventory.includes(staff.id)).map(staff => (
                  <InventoryItemRow 
                    key={staff.id} item={staff} 
                    isEquipped={activeStaffId === staff.id} onEquip={handleEquipStaff} 
                  />
                ))}
              </div>
            )}
            ...
            <div className="text-center">
              <button onClick={() => setGameState('title')} className="px-8 py-3 bg-amber-500 hover:bg-amber-400 text-white font-black text-lg rounded-full shadow-lg border-b-4 border-amber-600 active:border-b-0 active:translate-y-1 transition-all">
                タイトルへ戻る
              </button>
            </div>
          </div>
        ...
        {gameState === 'shop' && (
          <div className="pt-28 pb-12 px-2 md:px-10">
            ...
            {shopTab === 'facilities' && (
              <div className="grid gap-4 max-w-2xl mx-auto mb-10">
                <p className="text-center text-orange-600 font-bold mb-4 text-sm bg-orange-100 py-2 rounded-xl">
                  ※ 設備はゲームプレイ中に 1, 2 キーを押して使う消費アイテムです。<br className="md:hidden" />（装備解除は必要ありません）
                </p>
              ...
              <div className="grid gap-4 max-w-2xl mx-auto mb-10">
                {STAFF_ITEMS.map(staff => (
                  <ShopItemRow 
                    ...
                    onBuy={handleBuyStaff} actionText="雇用する"
                  />
                ))}
              </div>
            )}
        ...
        {gameState === 'playing' && (
          <div className="flex flex-col h-[500px] pt-16 relative">
            ...
            {/* イベント開始時の巨大バナー */}
            {currentEvent && (
              <div className="absolute top-1/2 left-1/2 z-50 pointer-events-none animate-event-banner flex flex-col items-center">
                <div className="text-4xl md:text-6xl font-black text-white drop-shadow-[0_0_15px_rgba(255,100,0,1)] whitespace-nowrap" style={{ WebkitTextStroke: '2px #c2410c' }}>
                  {getEventBannerName(currentEvent)}
                </div>
              </div>
            )}
            ...
            <div className="flex justify-between items-start mb-4 border-b-2 border-orange-100 pb-4 relative">
              <div className="flex items-center gap-6 md:gap-10">
                <div>
                  <p className="text-xs md:text-sm text-amber-600 font-bold">スコア(コイン)</p>
                  <p className="text-3xl md:text-5xl font-black text-orange-500 tabular-nums leading-none">{score}</p>
                </div>
                <div>
                  <p className="text-xs md:text-sm text-amber-600 font-bold">コンボ</p>
              ...
              <div className="flex flex-col items-end gap-2">
                <div className="text-right">
                  <p className="text-xs md:text-sm text-amber-600 font-bold">残り時間</p>
                  <p className={`text-4xl md:text-6xl font-black tabular-nums leading-none ${timeLeft <= 10 && timeLeft !== Infinity ? 'text-rose-500 animate-pulse' : 'text-orange-500'}`}>
                    {timeLeft === Infinity ? '∞' : timeLeft}<span className="text-2xl text-amber-700">{timeLeft === Infinity ? '' : '秒'}</span>
                  </p>
                </div>
                <button 
            ...
            <div className="flex-grow relative bg-orange-50/50 rounded-2xl overflow-hidden border-2 border-orange-100 shadow-inner">
              <div 
                className={`absolute inset-x-0 top-1/2 -translate-y-1/2 h-24 border-y-8 shadow-[inset_0_5px_15px_rgba(0,0,0,0.5)] transition-colors duration-500 ${isConveyorOn ? 'bg-slate-700 border-slate-600' : 'bg-blue-900 border-blue-700'}`}
                style={{
                  ...
                  animationPlayState: isConveyorOn ? 'running' : 'paused'
                }}
              />
              ...
              {currentBread && (
                <div 
                  ...
                  style={{
                    ...
                    animationDelay: currentBread.spawnedWhileStopped ? `-${animTime * 0.15}s` : '0s',
                    animationPlayState: isConveyorOn ? 'running' : 'paused'
                  }}
                  onAnimationEnd={handleMissWord}
                >
                  <div className="relative mb-3">
                  ...
                  <div className={`bg-white/95 backdrop-blur-sm px-6 py-3 rounded-2xl shadow-xl border-2 ${currentBread.word.isFire ? 'border-rose-500 bg-rose-50' : 'border-orange-200'} text-center whitespace-nowrap min-w-[200px] transition-all`}>
                    <div className={`text-lg md:text-xl font-bold ${currentBread.word.isFire ? 'text-rose-600' : 'text-amber-900'} mb-1 flex justify-center items-center gap-2`}>
                      {currentBread.word.isFire && <span className="animate-pulse">🔥</span>}
                      <span>{currentBread.word.name}</span>
                      <span className={`text-sm ${currentBread.word.isFire ? 'bg-rose-200 text-rose-800 border-rose-300' : 'bg-orange-100 text-orange-600 border-orange-200'} px-2 py-0.5 rounded-full border`}>
                        ¥{Math.floor(currentBread.word.price * currentEmotionMultiplier * (scoreBoost ? 2 : 1) * eventCoinMult)}
                      </span>
                    </div>
                    <div className={`text-2xl md:text-4xl font-mono font-black tracking-widest ${isOnFire ? 'opacity-30 blur-[1px]' : ''}`}>
                      ...
                      <span className={`${currentBread.word.isFire ? 'text-rose-500 border-rose-500' : 'text-orange-500 border-orange-500'} border-b-4 pb-1`}>
                        {currentBread.word.romaji.substring(currentBread.typedCount, currentBread.typedCount + 1)}
                      </span>
                      <span className={currentBread.word.isFire ? 'text-rose-800' : 'text-amber-800'}>
            ...
            {isOnFire && currentWater && (
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-40 bg-blue-50/95 backdrop-blur-sm border-4 border-blue-400 px-8 py-4 rounded-3xl shadow-[0_0_30px_rgba(59,130,246,0.6)] animate-pulse flex flex-col items-center">
                <h3 className="text-2xl font-black text-rose-600 mb-2 drop-shadow-sm flex items-center gap-2">
                  <span>🔥</span>辛い！水を飲んで鎮火しろ！<span>🔥</span>
                </h3>
                <div className="flex items-center gap-6">
                  <div className="text-7xl drop-shadow-md animate-bounce">{currentWater.word.emoji}</div>
                  <div className="bg-white px-8 py-4 rounded-2xl shadow-inner border-2 border-blue-200">
                    <div className="text-4xl font-mono font-black tracking-widest text-center">
                      <span className="text-blue-200">{currentWater.word.romaji.substring(0, currentWater.typedCount)}</span>
                      <span className="text-blue-500 border-b-4 border-blue-500 pb-1">{currentWater.word.romaji.substring(currentWater.typedCount, currentWater.typedCount + 1)}</span>
                      <span className="text-blue-800">{currentWater.word.romaji.substring(currentWater.typedCount + 1)}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="text-center pt-24 pb-12 flex flex-col items-center justify-center min-h-[400px]">
            ...
            {tradePhase === 'offer' && (
              <div className="animate-fade-in">
                <h2 className="text-2xl md:text-4xl font-black text-purple-100 mb-8 bg-slate-800 px-8 py-6 rounded-3xl border-4 border-purple-900 shadow-[0_0_30px_rgba(88,28,135,0.6)]">
                  「クックック… 取引しないか？」
                </h2>
                <div className="flex flex-col md:flex-row justify-center gap-4">
                  <button onClick={handleTradeRefuse} className="px-8 py-4 bg-slate-700 hover:bg-slate-600 text-white font-black text-xl rounded-full shadow-lg border-b-4 border-slate-900 active:border-b-0 active:translate-y-1 transition-all">
                    渡さない
                  </button>
                  <button onClick={handleTradeAccept} className="px-8 py-4 bg-purple-700 hover:bg-purple-600 text-purple-100 font-black text-xl rounded-full shadow-[0_0_20px_rgba(168,85,247,0.4)] border-b-4 border-purple-900 active:border-b-0 active:translate-y-1 transition-all">
                    一番レアなものを渡す
                  </button>
                </div>
              </div>
            ...
            {tradePhase === 'no_item' && (
              <div className="animate-fade-in">
                <h2 className="text-2xl md:text-3xl font-black text-purple-100 mb-8 bg-slate-800 px-8 py-6 rounded-3xl border-4 border-purple-900 shadow-[0_0_30px_rgba(88,28,135,0.6)]">
                  「なんだ、取引できるような物を<br className="md:hidden" />何も持っていないじゃないか…」
                </h2>
                <button onClick={handleTradeFinish} className="px-8 py-4 bg-slate-700 hover:bg-slate-600 text-white font-black text-xl rounded-full shadow-lg border-b-4 border-slate-900 active:border-b-0 active:translate-y-1 transition-all">
                  次へ
                </button>
              </div>
            )}
            ...
            {tradePhase === 'done' && (
              <div className="animate-fade-in">
                <h2 className="text-2xl md:text-4xl font-black text-purple-100 mb-8 bg-slate-800 px-8 py-6 rounded-3xl border-4 border-purple-900 shadow-[0_0_30px_rgba(88,28,135,0.6)] leading-relaxed">
                  「クックック…<br/>
                  <span className="text-yellow-400 drop-shadow-[0_0_10px_rgba(250,204,21,0.8)]">『{gainedTradeReward}』</span> をやろう。<br/>
                  さらばだ。」
                </h2>
                <button onClick={handleTradeFinish} className="px-8 py-4 bg-slate-700 hover:bg-slate-600 text-white font-black text-xl rounded-full shadow-lg border-b-4 border-slate-900 active:border-b-0 active:translate-y-1 transition-all">
                  リザルトへ
        ...
        {gameState === 'result' && (
          <div className="text-center pt-24 pb-12">
            <h2 className="text-4xl font-black text-amber-800 mb-2">
              {course.noCoin ? '練習終了！🥖' : isRetired ? 'リタイア！🥖' : 'タイムアップ！🥖'}
            </h2>
            <p className="text-xl font-bold text-orange-600 mb-4">コース: {course.name}</p>
            
            {!course.noCoin ? (
              isRetired ? (
                <div className="inline-block bg-slate-100 border-2 border-slate-400 px-6 py-2 rounded-full mb-8 shadow-md">
                  <p className="text-lg font-black text-slate-700">リタイアのためコイン獲得なし</p>
                </div>
              ) : (
              ...
              <div className="inline-block bg-teal-100 border-2 border-teal-400 px-6 py-2 rounded-full mb-8 shadow-md">
                <p className="text-lg font-black text-teal-700">練習お疲れ様{displayStaff.suffix}！</p>
              </div>
            )}
